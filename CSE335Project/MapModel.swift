...
class MapModel: ObservableObject {
    ...
    func loadProperties() {
        ...
        let urlString = """
        https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=\(location)&radius=3000&type=apartment&keyword=rental|apartment&key=\(apiKey)
        """
        ...
        URLSession.shared.dataTask(with: url) { data, _, error in
            ...
            do {
                ...
                DispatchQueue.main.async {
                    self.properties = decoded.results
                        .filter { !$0.name.lowercased().contains("hotel") }
                        .map {
                            Property(
                                title: $0.name,
